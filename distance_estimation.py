import cv2 as cv
import numpy as np
import yaml
import time

# --- 1) LOAD CAMERA CALIBRATION -------------------
# calib.yaml should contain:
#   K:   3x3 intrinsic matrix
#   dist: distortion coefficients [k1,k2,p1,p2,(k3)]
with open('calib.yaml') as f:
    data = yaml.safe_load(f)
K = np.array(data['K'])
dist = np.array(data['dist'])

# Optional: if you have wheel-encoder or IMU odometry giving
# the true translation (in meters) between frames, set this:
# e.g. odo_dist = 0.05  # car moved 5 cm between frames
odo_dist = None


# --- 2) SETUP FEATURE DETECTOR & MATCHER -----------
orb    = cv.ORB_create(2000)
bf     = cv.BFMatcher(cv.NORM_HAMMING, crossCheck=False)

# --- 3) START VIDEO STREAM -------------------------
cap = cv.VideoCapture(0)
if not cap.isOpened():
    raise RuntimeError("Cannot open camera")

# read first frame
ret, prev_raw = cap.read()
if not ret:
    raise RuntimeError("Cannot read first frame")
prev_gray = cv.cvtColor(prev_raw, cv.COLOR_BGR2GRAY)
prev_gray = cv.undistort(prev_gray, K, dist)
kp_prev, des_prev = orb.detectAndCompute(prev_gray, None)

print('debug')

while True:
    t0 = time.time()
    ret, raw = cap.read()
    if not ret:
        break

    # Undistort & grayscale
    gray = cv.cvtColor(raw, cv.COLOR_BGR2GRAY)
    gray = cv.undistort(gray, K, dist)

    # 4) FEATURE DETECTION & MATCHING ----------------
    kp, des = orb.detectAndCompute(gray, None)
    if des is None or des_prev is None or len(des_prev)<10:
        # not enough features, skip
        prev_gray, kp_prev, des_prev = gray, kp, des
        continue

    # KNN match + ratio test
    matches = bf.knnMatch(des_prev, des, k=2)
    good = []
    for m,n in matches:
        if m.distance < 0.75*n.distance:
            good.append(m)
    if len(good) < 8:
        prev_gray, kp_prev, des_prev = gray, kp, des
        continue

    pts_prev = np.float32([kp_prev[m.queryIdx].pt for m in good])
    pts      = np.float32([   kp[m.trainIdx].pt    for m in good])

    # 5) ESSENTIAL MATRIX & POSE RECOVERY ------------
    E, mask = cv.findEssentialMat(pts_prev, pts, K,
                                  method=cv.RANSAC, prob=0.999, threshold=1.0)
    if E is None:
        prev_gray, kp_prev, des_prev = gray, kp, des
        continue

    _, R, t, mask_pose = cv.recoverPose(E, pts_prev, pts, K)

    # If you have odometry: compute scale = odo_dist / ||t||
    scale = 1.0
    if odo_dist is not None:
        raw_t   = np.linalg.norm(t)
        if raw_t > 1e-6:
            scale = odo_dist / raw_t

    # 6) TRIANGULATION -------------------------------
    # Projection matrices in *camera* frame
    P1 = np.hstack((np.eye(3), np.zeros((3,1))))
    P2 = np.hstack((R, scale*t))

    # convert to 3x4
    P1 = K.dot(P1)
    P2 = K.dot(P2)

    # triangulate (returns 4×N homogeneous coordinates)
    pts4d = cv.triangulatePoints(P1, P2, pts_prev.T, pts.T)
    pts3d = (pts4d[:3] / pts4d[3]).T  # N×3

    # compute distances
    dists = np.linalg.norm(pts3d, axis=1)
    min_dist = np.min(dists)

    # 7) DISPLAY -------------------------------------
    text = f"Closest Obj Dist: {min_dist:.2f} {'m' if odo_dist else 'units'}"
    cv.putText(raw, text, (20,30), cv.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), 2)
    cv.imshow('Live Distance', raw)

    # shift frames
    prev_gray, kp_prev, des_prev = gray, kp, des

    # break on ESC
    if cv.waitKey(1) & 0xFF == 27:
        break

    # (optional) maintain ~20 FPS
    dt = time.time() - t0
    if dt<0.05: time.sleep(0.05-dt)


cap.release()
cv.destroyAllWindows()